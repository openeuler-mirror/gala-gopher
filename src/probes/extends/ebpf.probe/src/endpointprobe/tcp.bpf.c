...
static __always_inline void add_sock_and_tgid(const struct sock *sk, enum socket_role_e role, struct tcp_listen_val_s *v)
{
    ...
    info.role = role;
    __builtin_memcpy(info.comm, v->comm, sizeof(v->comm));
...
static __always_inline void add_sock_by_listen_sk(const struct sock *sk, struct sock_info_s *info)
{
    ...
    __builtin_memcpy(new_info.comm, info->comm, sizeof(info->comm));
...
static __always_inline void report_synack_sent_evt(void *ctx, const struct sock* sk, const struct request_sock *req, bool retran)
{
    ...
    evt.tgid = info->tgid;
    __builtin_memcpy(evt.comm, info->comm, sizeof(info->comm));
 ...
 */
static __always_inline bool get_toa_data(const struct tcphdr *th, int af, struct tcp_socket_event_s *evt)
{
    int length;
    ...
    for (int i = 0; i <= MAX_TCPOPT_LEN - sizeof(struct toa_opt); i++) {
        ...
        if (opcode == TCPOPT_TOA && opsize == TCPOLEN_TOA) {
            __builtin_memcpy(&evt->toa_client_ipaddr.ip, buffer + i + 4, sizeof(evt->toa_client_ipaddr.ip));
        ...
        if (af == AF_INET6 && opcode == TCPOPT_TOA_V6 && opsize == TCPOLEN_TOA_V6) {
            if (i > length - TCPOLEN_TOA_V6 || i > MAX_TCPOPT_LEN - TCPOLEN_TOA_V6)
                return false;
            __builtin_memcpy(evt->toa_client_ipaddr.ip6, buffer + i + 4, IP6_LEN);
...
KPROBE(tcp_conn_request, pt_regs)
{
    ...
    struct sock_info_s* info = lkup_sock((const struct sock *)sk);
    struct tcp_listen_val_s *val = NULL;
    if (info && info->role == TCP_LISTEN_SK) {
        tgid = info->tgid;
        is_multi = info->is_multi;
    } else {
        val = get_proc_info_by_listen_sk((const struct sock *)sk);
        if (val == NULL) {
            goto end;
        }
        tgid = val->proc_id;
    ...
    if (val)
        __builtin_memcpy(evt.comm, val->comm, sizeof(val->comm));
