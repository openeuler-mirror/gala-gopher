...
int function_stack_trace(struct bpf_perf_event_data *ctx)
{
    ...
    if (py_sample) {
        ...
        if (is_stackmap_a) {
            (void)bpfbuf_output(ctx, &stackmap_perf_a, &py_sample->event, sizeof(py_sample->event));
        } else {
            (void)bpfbuf_output(ctx, &stackmap_perf_b, &py_sample->event, sizeof(py_sample->event));
        }
        return 0;
    }
