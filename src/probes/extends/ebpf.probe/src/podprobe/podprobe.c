...
static enum id_ret_t get_pod_container_id(char *cgrp_path, char *pod_id, char *con_id)
{
    ...
    if (i + p - cgrp_path == full_path_len) {
        return ID_POD_ONLY; // only pod cgroup, not container cgroup
    }
...
static struct pods_hash_t *add_one_pod(char *pod_id, char *con_id)
{
...
static void cgrp_mk_process(char *cgrp_path)
{
    ...
    pod = add_one_pod(pod_id, con_id);
    if (pod == NULL) {
        return;
    }

    if (id_ret == ID_CON_POD) {
