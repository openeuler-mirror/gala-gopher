...
static void ippro_to_str(unsigned short protocol, char *type_str, size_t type_str_len)
{
    switch (protocol) {
        case IPPROTO_IP:
            snprintf(type_str, type_str_len, "%s", "IP");
            break;
        case IPPROTO_TCP:
            snprintf(type_str, type_str_len, "%s", "TCP");
            break;
        case IPPROTO_UDP:
            snprintf(type_str, type_str_len, "%s", "UDP");
            break;
        case IPPROTO_IPV6:
            snprintf(type_str, type_str_len, "%s", "IPV6");
            break;
        default:
            snprintf(type_str, type_str_len, "%s", "Err");
    }
    return;
}
...
static void update_ipvs_collect_map(const struct link_key *k, unsigned short protocol, const struct ip *laddr, __u16 lport, int map_fd)
{
...
static void pull_probe_data(int fd, int collect_fd)
{
    ...
    char ip_pro_str[INET6_ADDRSTRLEN];
    ...
    while (bpf_map_get_next_key(fd, &key, &next_key) == 0) {
        ...
        if (ret == 0) {
            ippro_to_str(value.protocol, ip_pro_str, INET6_ADDRSTRLEN);
            ...
            LVS_DEBUG("LVS new connect protocol[%s] type[%s] c[%s:%d]--v[%s:%d]--l[%s:%d]--s[%s:%d] state[%d]. \n",
                ip_pro_str,
                ...
                value.state);
            /* update collect map */
            update_ipvs_collect_map(&next_key, value.protocol, &value.l_addr, value.l_port, collect_fd);
        }
