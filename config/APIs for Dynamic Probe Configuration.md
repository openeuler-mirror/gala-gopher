...
For details about how to use the command line tool for dynamic probe configuration, see Dynamically Configuring Probes Using the Command Line Tool. The following describes how to dynamically configure probes using the RESTful API.
...
1.	The cmd field is used to configure basic probe attributes, including probe collection subitems.
2.	The snoopers field is used to configure the observation scope from five dimensions: process ID, process name, Pod ID, container ID, and container name. In addition, it supports extended label matching.
3.	The params field is used to configure probe running parameters.
4.	The state field is used to configure the probe running status, that is, to start or stop the probe.
...
Collection Feature	Description	Collection Subitems	Description
flamegraph	On-line performance flame graph observation capability	oncpu, offcpu, mem	oncpu: collects the status of application threads running on the CPU, helping identify how threads consume CPU resources and pinpoint time-consuming function calls. 
offcpu: collects the status of application threads that are not running on the CPU, helping analyze operations (such as waiting for I/O or locks) that cause threads to enter the offcpu state. 
mem: collects the memory allocation stack of application threads within the queried time period to determine memory usage.
l7	Application layer (layer 7) protocol observation capability	l7_bytes_metrics, l7_rpc_metrics, l7_rpc_trace	l7_bytes_metrics: collects the number of data bytes and data packets received and sent by application threads. 
l7_rpc_metrics: collects various metrics such as the number of requests received, number of responses sent, request throughput, response throughput, average latency, total latency, and error rate for application threads. 
l7_rpc_trace: not supported currently.
TCP	TCP exception and status observation capability	tcp_abnormal, tcp_rtt, tcp_windows, tcp_rate, tcp_srtt, tcp_sockbuf, tcp_stats,tcp_delay	tcp_abnormal: TCP exception information 
tcp_rtt: TCP connection data transmission round-trip time
tcp_windows: TCP window information 
tcp_rate: TCP transmission rate 
tcp_srtt: TCP connection Smoothed Round Trip Time (SRTT) 
tcp_sockbuf: size of the buffer for receiving and sending data 
tcp_stats: TCP connection status 
tcp_delay: TCP transmission delay
socket	Socket (TCP/UDP) exception observation capability	tcp_socket, udp_socket	tcp_socket: TCP socket information 
udp_socket: UDP socket information
io	Block layer I/O observation capability	io_trace, io_err, io_count, page_cache	io_trace: number of I/O requests
io_err: I/O error information 
io_count: number of read and write bytes of I/O operations 
page_cache: I/O cache information
proc	Process system call, I/O, DNS, and VFS observation capabilities	proc_syscall, proc_fs, proc_io, proc_dns,proc_pagecache, proc_net, proc_offcpu, proc_ioctl	proc_syscall: process system call information
proc_fs: process file system information 
proc_io: process I/O information 
proc_dns: DNS access monitoring 
proc_pagecache: process memory page information
proc_net: statistics on the duration for a process to send and receive network packets 
proc_offcpu: statistics on the process IO_wait and offcpu duration 
proc_ioctl: statistics on the process ioctl size and duration
jvm	JVM layer GC, thread, memory, and cache observation capabilities	N/A	N/A
ksli	Redis performance SLI (access latency) observation capability	N/A	N/A
postgre_sli	PostgreSQL database performance SLI (access latency) observation capability	N/A	N/A
opengauss_sli	openGauss access throughput observation capability	N/A	N/A
dnsmasq	DNS session observation capability	N/A	N/A
LVS	LVS session observation capability	N/A	N/A
Nginx	Nginx layer 4/layer 7 session observation capability	N/A	N/A
HAProxy	HAProxy layer 4/layer 7 session observation capability	N/A	N/A
Kafka	Kafka producer/consumer topic observation capability	N/A	N/A
baseinfo	Basic system information	cpu, mem, nic, disk, net, fs, proc, host, con	cpu: CPU performance
mem: memory performance
nic: NIC performance 
disk: disk performance 
net: protocol stack statistics 
fs: file system information
proc: process information 
host: host information 
con: container information
virt	Virtualization management information	N/A	N/A
tprofiling	Thread-level performance profiling observation capability	oncpu, syscall_file, syscall_net, syscall_lock, syscall_sched	oncpu: status of application threads running on the CPU, helping identify how threads consume CPU resources and pinpoint time-consuming function calls. 
syscall_file: information about the file system called by the system 
syscall_net: performance of the network called by the system. 
syscall_lock: number of system call deadlocks 
syscall_sched: number of system calls
Container	Container information	N/A	N/A
...
Configuration for the Flame Graph Probe
...
Configuration for the Application Layer 7 Protocol Probe
...
6.	The state parameter specifies the probe status. To start a probe, set the state parameter to running. To stop a probe, set the state parameter to stopped.
Configuration for the TCP Exception and Status Observation Probe
...
•	<probe_name>: specifies the probe name. For details about the value range, see Probe Configuration APIs.
•	<probe_config>: specifies probe configuration information in JSON format. The content is the same as that of the RESTful API. For details, see Probe Configuration APIs.
